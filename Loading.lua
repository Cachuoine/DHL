--================================================================
-- FishHub Premium - Cinematic Anchor Spinner & Cyberpunk Key UI
--================================================================
...
local function loadMainUI()
    pcall(function()
        local code = game:HttpGet("https://raw.githubusercontent.com/Cachuoine/DHL/refs/heads/main/UI.lua")
...
--============= 1. CINEMATIC LOADING SCREEN (MÓC NEO XOAY ĐÚNG 1 VÒNG) ==============
...
-- Biểu tượng Móc Neo đại diện cho chủ đề FishHub
local AnchorIcon = Instance.new("TextLabel", LoadBg)
AnchorIcon.Size = UDim2.new(0, 50, 0, 50)
AnchorIcon.Position = UDim2.new(0.5, -25, 0.2, 0)
AnchorIcon.BackgroundTransparency = 1
AnchorIcon.Font = Enum.Font.GothamBold
AnchorIcon.Text = "⚓"
AnchorIcon.TextColor3 = C_CYAN
...
-- Animation móc neo xoay tròn đúng 1 vòng (360 độ)
local spinAnchor = TweenService:Create(AnchorIcon, TweenInfo.new(1, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {Rotation = 360})
spinAnchor:Play()

--============= 2. KEY SYSTEM INTERFACE (TRANG TRÍ THÊM HIỆU ỨNG CYBERPUNK - KHÔNG AVATAR) ==============
local KeyMain = Instance.new("Frame", ScreenGui)
KeyMain.Size = UDim2.new(0, 440, 0, 300)
...
-- Dải đèn Neon trang trí chạy dọc viền trên giao diện Key System tạo điểm nhấn Cyberpunk
local TopNeonBar = Instance.new("Frame", KeyMain)
TopNeonBar.Size = UDim2.new(0, 80, 0, 3)
TopNeonBar.Position = UDim2.new(0.5, -40, 0, 0)
TopNeonBar.BackgroundColor3 = C_PURPLE
TopNeonBar.BackgroundTransparency = 1
round(TopNeonBar, 2)

-- Nút đóng UI ở góc trên bên phải (dấu nhân ×) với hiệu ứng xoay góc ngầu
local BtnClose = Instance.new("TextButton", KeyMain)
BtnClose.Size = UDim2.new(0, 30, 0, 30)
BtnClose.Position = UDim2.new(1, -38, 0, 14)
...
-- Icon biểu tượng nhỏ cạnh tiêu đề chính
local TitleIcon = Instance.new("TextLabel", KeyMain)
TitleIcon.Size = UDim2.new(0, 30, 0, 45)
TitleIcon.Position = UDim2.new(0, 20, 0, 15)
TitleIcon.BackgroundTransparency = 1
TitleIcon.Font = Enum.Font.GothamBold
TitleIcon.Text = "🛡️"
TitleIcon.TextSize = 18
TitleIcon.TextTransparency = 1
...
-- Đường kẻ phân cách sang trọng dưới tiêu đề
local Divider = Instance.new("Frame", KeyMain)
Divider.Size = UDim2.new(0.85, 0, 0, 1)
Divider.Position = UDim2.new(0.075, 0, 0.23, 0)
Divider.BackgroundColor3 = Color3.fromRGB(45, 52, 75)
Divider.BackgroundTransparency = 1

local BoxWrap = Instance.new("Frame", KeyMain)
BoxWrap.Size = UDim2.new(0.85, 0, 0, 46)
BoxWrap.Position = UDim2.new(0.075, 0, 0.30, 0)
...
BtnCheck.Size = UDim2.new(0.4, 0, 0, 42)
...
BtnGet.Size = UDim2.new(0.4, 0, 0, 42)
...
-- Thêm hộp thông báo nhỏ (Info Box) trang trí dưới nút xác thực
local InfoBox = Instance.new("Frame", KeyMain)
InfoBox.Size = UDim2.new(0.85, 0, 0, 38)
InfoBox.Position = UDim2.new(0.075, 0, 0.74, 0)
InfoBox.BackgroundColor3 = Color3.fromRGB(16, 20, 32)
InfoBox.BackgroundTransparency = 1
round(InfoBox, 8)

local InfoStroke = Instance.new("UIStroke", InfoBox)
InfoStroke.Color = Color3.fromRGB(35, 42, 65)
...
InfoText.Text = "💡 Tip: Keys remain valid for 24 hours after activation."
InfoText.TextColor3 = Color3.fromRGB(130, 140, 170)
InfoText.TextSize = 10
InfoText.TextTransparency = 1
InfoText.TextXAlignment = Enum.TextXAlignment.Left

local SettingsLabel = Instance.new("TextLabel", KeyMain)
SettingsLabel.Size = UDim2.new(0.85, 0, 0, 20)
SettingsLabel.Position = UDim2.new(0.075, 0, 0.88, 0)
SettingsLabel.BackgroundTransparency = 1
SettingsLabel.Font = Enum.Font.GothamBold
SettingsLabel.Text = "SYSTEM CONFIGURATION & SECURITY"
SettingsLabel.TextColor3 = Color3.fromRGB(100, 110, 140)
SettingsLabel.TextSize = 9
SettingsLabel.TextTransparency = 1

-- Kiểm tra Key đã lưu từ trước
task.spawn(function()
    spinAnchor.Completed:Wait() -- Chờ móc neo xoay tròn đúng 1 vòng xong mới tiếp tục
    ...
    -- Hiển thị bảng Key với Animation Elastic / Back cực ngầu
    TweenService:Create(LoadBg, TweenInfo.new(0.4), {Size = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 1}):Play()
    task.wait(0.3)
    LoadBg.Visible = false

    KeyMain.Visible = true
    TweenService:Create(KeyMain, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {BackgroundTransparency = 0.08}):Play()
    TweenService:Create(MainStroke, TweenInfo.new(0.5), {Transparency = 0.3}):Play()
    TweenService:Create(TopNeonBar, TweenInfo.new(0.5), {BackgroundTransparency = 0}):Play()
    TweenService:Create(BtnClose, TweenInfo.new(0.5), {BackgroundTransparency = 0.2, TextTransparency = 0}):Play()
    TweenService:Create(TitleIcon, TweenInfo.new(0.5), {TextTransparency = 0}):Play()
    TweenService:Create(Title, TweenInfo.new(0.5), {TextTransparency = 0}):Play()
    TweenService:Create(Divider, TweenInfo.new(0.5), {BackgroundTransparency = 0}):Play()
    TweenService:Create(BoxWrap, TweenInfo.new(0.5), {BackgroundTransparency = 0.2}):Play()
    TweenService:Create(BoxStroke, TweenInfo.new(0.5), {Transparency = 0}):Play()
    TweenService:Create(InputKey, TweenInfo.new(0.5), {TextTransparency = 0}):Play()
    TweenService:Create(BtnCheck, TweenInfo.new(0.5), {BackgroundTransparency = 0, TextTransparency = 0}):Play()
    TweenService:Create(BtnGet, TweenInfo.new(0.5), {BackgroundTransparency = 0, TextTransparency = 0}):Play()
    TweenService:Create(InfoBox, TweenInfo.new(0.5), {BackgroundTransparency = 0.2}):Play()
    TweenService:Create(InfoStroke, TweenInfo.new(0.5), {Transparency = 0}):Play()
    TweenService:Create(InfoText, TweenInfo.new(0.5), {TextTransparency = 0}):Play()
    TweenService:Create(SettingsLabel, TweenInfo.new(0.5), {TextTransparency = 0}):Play()
end)

-- Hiệu ứng Hover & Xử lý tương tác siêu mượt
BtnCheck.MouseEnter:Connect(function() TweenService:Create(BtnCheck, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(0, 210, 230), Size = UDim2.new(0.4, 0, 0, 44)}):Play() end)
BtnCheck.MouseLeave:Connect(function() TweenService:Create(BtnCheck, TweenInfo.new(0.2), {BackgroundColor3 = C_CYAN, Size = UDim2.new(0.4, 0, 0, 42)}):Play() end)

BtnGet.MouseEnter:Connect(function() TweenService:Create(BtnGet, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(50, 58, 90), Size = UDim2.new(0.4, 0, 0, 44)}):Play() end)
BtnGet.MouseLeave:Connect(function() TweenService:Create(BtnGet, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(35, 40, 65), Size = UDim2.new(0.4, 0, 0, 42)}):Play() end)

BtnClose.MouseEnter:Connect(function() TweenService:Create(BtnClose, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(220, 38, 38), Rotation = 90}):Play() end)
BtnClose.MouseLeave:Connect(function() TweenService:Create(BtnClose, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(239, 68, 68), Rotation = 0}):Play() end)

-- Nút nhân (✕) bấm vào sẽ có hiệu ứng thu nhỏ văng ra ngoài rồi hủy hoàn toàn UI cực ngầu
BtnClose.MouseButton1Click:Connect(function()
    TweenService:Create(BlurEffect, TweenInfo.new(0.4), {Size = 0}):Play()
    
    local animOut = TweenService:Create(KeyMain, TweenInfo.new(0.35, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Size = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 1, Rotation = -15})
...
BtnCheck.MouseButton1Click:Connect(function()
    ...
    if checkKeyValid(userKey) then
        ...
        Title.Text = (userKey == ADMIN_KEY) and "ADMIN ACCESS GRANTED!" or "ACCESS GRANTED!"
